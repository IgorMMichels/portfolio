...
export function DottedSurface({ className, position = 'absolute', ...props }: DottedSurfaceProps) {
  ...
  useEffect(() => {
    ...
    const checkTheme = () => {
      ...
      // Dark mode typically has low RGB values
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(isDark ? 0x05060a : 0xffffff, 2000, 10000);
  ...
  return (
    <div
      ref={containerRef}
      className={cn(`pointer-events-none ${position} inset-0 -z-10`, className)}
      {...props}
    />
  );
}
