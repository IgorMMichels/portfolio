...
const Card = React.forwardRef<HTMLDivElement, { item: iCardItem; index: number }>(
  ({ item, index }, ref) => {
    return (
      <div
        ref={ref}
        className="card-parallax-item absolute flex flex-col md:flex-row items-stretch justify-between w-[85vw] max-w-5xl rounded-2xl overflow-hidden shadow-2xl border border-white/10 group"
        style={{
        ...
        <div className="flex-1 p-6 md:p-10 lg:p-12 flex flex-col justify-center gap-4 relative z-20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 transform translate-y-4 group-hover:translate-y-0">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {item.techStack.map((tech) => (
              <span
                key={tech}
                className="px-3 py-1.5 rounded-full text-xs font-medium bg-black/40 border border-white/10"
